...
{
  ASSERT(thread_mlfqs);
  int f_load_avg100= real_mixproduct(f_load_avg,100);
  int r=real_int(f_load_avg100);
  return r;
}
...
{
  ASSERT(thread_mlfqs);
  int f_recent_cpu100 = real_mixproduct (thread_current ()->f_recent_cpu,100);
  int cpu = real_int (f_recent_cpu100);
  return cpu;
}
...
/*Call this function every 4th tick to update the priority*/
void
refresh_priority(struct thread *t)
{
 ASSERT(thread_mlfqs);
 int rec_cpu=real_int(real_mixdiv(t->f_recent_cpu,4)); 
 int new_priority=PRI_MAX - rec_cpu - ((t->nice)*2);
 enum intr_level old_level = intr_disable();
 ASSERT (intr_get_level() == INTR_OFF);
...
/*call 2 below functions every second and update f_recent_cpu for each thread*/ 
void
refresh_load_avg()
{
 ASSERT(thread_mlfqs);
 size_t n_ready_threads=list_size(&ready_list);
 
 if(thread_current () != idle_thread)
   n_ready_threads++;
   
 int f_59=int_real(59);
 int f_59div60=real_mixdiv(f_59,60);
 int f_1=int_real(1);
 int f_1div60=real_mixdiv(f_1,60);
 int f_temp1=real_product(f_59div60,f_load_avg);
 int f_temp2=real_mixproduct(f_1div60,n_ready_threads);
 f_load_avg=real_sum(f_temp1,f_temp2);
}
...
refresh_recent_cpu(struct thread *t)
{
 ...
 int f_temp1=real_mixproduct(f_load_avg,2);
 int f_temp2=real_div(f_temp1,real_mixsum(f_temp1,1));
 f_temp1=real_product(f_temp2,t->f_recent_cpu);
 f_temp2=real_mixsum(f_temp1,t->nice);
 
 enum intr_level old_level = intr_disable();
 ASSERT (intr_get_level() == INTR_OFF);
 t->f_recent_cpu=f_temp2;
 intr_set_level (old_level);
